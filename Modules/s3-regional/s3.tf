...
resource "aws_s3_bucket" "vpc_logs" {
  bucket = "${var.env}-${var.aws_region}-vpc-flow-logs-${data.aws_caller_identity.current.account_id}"
  force_destroy = true
...
resource "aws_s3_bucket" "alb_logs" {
  bucket = "${var.env}-${var.aws_region}-alb-logs-${data.aws_caller_identity.current.account_id}"
  force_destroy = true
